...
(defn parse-passport [ppt]
  (->> ppt
       (re-seq #"(\w+):(\S+)")
       (into {} (map (fn [[_ kwd val]] [(keyword kwd) val])))))

(defn to-maps [input] (map parse-passport (str/split input #"\R\R")))
...
(defn valid-passport? [m]
  (every? (fn [[_ validator]] (validator m)) tag-validators))

(defn validator [key regex callback]
  (fn [m]
    (try
      (->> (get m key)
           (re-find regex)
...
(def tag-validators2
  {:byr
   (validator :byr #"^\d{4}$" (fn [byr] (<= 1920 (Long/parseLong byr) 2002)))
   :iyr
   (validator :iyr #"^\d{4}$" (fn [iyr] (<= 2010 (Long/parseLong iyr) 2020)))
   :eyr
   (validator :eyr #"^\d{4}$" (fn [eyr] (<= 2020 (Long/parseLong eyr) 2030)))
   :hgt
   (validator :hgt #"^(\d+)(cm|in)$"
              (fn [[_ val unit]]
                (condp = unit
                  "cm" (<= 150 (Long/parseLong val) 193)
                  "in" (<= 59 (Long/parseLong val) 76)
                  :else false)))
   :hcl
   (validator :hcl #"^#[0-9a-z]{6}$" identity)
   :ecl
   (validator :ecl #"^(amb|blu|brn|gry|grn|hzl|oth)$" identity)
   :pid
   (validator :pid #"^\d{9}$" identity)})


(defn valid-passport2? [m]
  (every? (fn [[_ validator]] (validator m)) tag-validators2))


(defn count-with [input f]
  (count (filter f (to-maps input))))
...
(count-with input valid-passport?)

(count-with invalid-input valid-passport2?)
(count-with valid-input valid-passport2?)
(count-with input valid-passport2?)
